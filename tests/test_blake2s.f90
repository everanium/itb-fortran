! test_blake2s.f90 -- BLAKE2s-focused low-level cipher coverage.
!
! BLAKE2s ships at a single hash width (256 in ITB's seed-width
! taxonomy; the underlying digest is 32 bytes). The test program
! covers the canonical cross-binding shape: roundtrip across the
! 128 / 256 / 512 nonce-bit settings, Triple Ouroboros roundtrip
! across the same axis, the authenticated counterpart for both
! Single and Triple (with a tamper step that confirms a flipped bit
! inside the dynamic header region is rejected as STATUS_MAC_FAILURE),
! a persistence sweep that round-trips a saved seed via
! from_components / hash_key, a plaintext-size sweep across
! small + medium + large payloads, and an invariants block (width,
! hash-key length).
!
...
program test_blake2s
  ...
  character(*), parameter :: TEST_NAME = "test_blake2s"
  character(*), parameter :: HASH_NAME = "blake2s"
...
contains
  ...
  function token_bytes(n) result(out)
    ...
    integer(c_int64_t), save :: ctr = int(z'2EADBEEFCAFEBABE', c_int64_t)
    integer(c_int64_t) :: state
    integer :: i
    integer(c_int64_t), parameter :: GOLD = int(z'2E3779B97F4A7C15', c_int64_t)
