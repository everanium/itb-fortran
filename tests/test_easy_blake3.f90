! test_easy_blake3.f90 -- BLAKE3 Easy Mode encryptor coverage.
!
! Mirrors the C binding's test_easy_blake3.c at the Easy Mode level.
! BLAKE3 ships at a single hash width (256). The test exercises Single
! Ouroboros + Triple Ouroboros construction via `new_itb_encryptor`,
! the read-only accessor surface, plain encrypt / decrypt round-trip,
! and authenticated encrypt / decrypt round-trip with a tampered-
! ciphertext rejection step.
!
...
program test_easy_blake3
  ...
  character(*), parameter :: TEST_NAME = "test_easy_blake3"
...
contains
  ...
  function pseudo_plaintext(n) result(p)
    ...
    allocate (p(n))
    do i = 1, n
      p(i) = int(iand(int(i - 1, c_int64_t) * 23_c_int64_t + 41_c_int64_t, &
