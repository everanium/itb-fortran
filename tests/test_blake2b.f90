! test_blake2b.f90 -- BLAKE2b-focused low-level cipher coverage.
!
! BLAKE2b ships at two ITB hash widths: blake2b256 (digest 32, fixed
! key 32) and blake2b512 (digest 64, fixed key 64). The test program
! covers the canonical cross-binding shape across both widths:
...
program test_blake2b
  ...
  character(*), parameter :: TEST_NAME = "test_blake2b"

  ! Canonical (hash, ITB seed-width, fixed-key length) tuples for the
  ! two BLAKE2b widths.
  integer, parameter :: NUM_HASHES = 2
  character(len=10), parameter :: HASH_NAMES(NUM_HASHES) = &
                          [character(len=10) :: "blake2b256", "blake2b512"]
...
contains
  ...
  function token_bytes(n) result(out)
    ...
    integer(c_int64_t), save :: ctr = int(z'6EADBEEFCAFEBABE', c_int64_t)
    integer(c_int64_t) :: state
    integer :: i
    integer(c_int64_t), parameter :: GOLD = int(z'6E3779B97F4A7C15', c_int64_t)
