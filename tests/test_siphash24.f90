! test_siphash24.f90 -- SipHash-2-4-focused low-level cipher coverage.
!
! SipHash-2-4 ships at a single hash width (128) and has no internal
! fixed key -- the keying material is the per-call seed components --
! so the hash-key length contract is 0 bytes and the persistence path
! stores an empty hash key. The test program covers the canonical
! cross-binding shape: roundtrip across the 128 / 256 / 512 nonce-bit
! settings, Triple Ouroboros roundtrip across the same axis, the
...
! trips a saved seed via from_components / hash_key (the empty hash
! key is exercised here), a plaintext-size sweep across small +
! medium + large payloads, and an invariants block (width, hash-key
! length).
!
...
program test_siphash24
  ...
  character(*), parameter :: TEST_NAME = "test_siphash24"
  character(*), parameter :: HASH_NAME = "siphash24"
  integer,      parameter :: HASH_WIDTH = 128
  integer,      parameter :: HASH_KEY_LEN = 0
...
contains
  ...
  function token_bytes(n) result(out)
    ...
    integer(c_int64_t), save :: ctr = int(z'5EADBEEFCAFEBABE', c_int64_t)
    integer(c_int64_t) :: state
    integer :: i
    integer(c_int64_t), parameter :: GOLD = int(z'5E3779B97F4A7C15', c_int64_t)
  ...
  subroutine test_roundtrip_across_nonce_sizes()
    ...
    do i = 1, size(NONCE_SIZES)
      ...
      ! SipHash-2-4 carries no fixed hash key, so the binding's
      ! `hash_width = 128` floor governs key_bits validation. 1024
      ! bits divides cleanly into the 128-bit hash width.
  ...
  subroutine test_persistence_across_nonce_sizes()
    ...
    do ki = 1, size(CANDIDATE_KB)
      ...
      do i = 1, size(NONCE_SIZES)
        ...
        ds_key = ds%hash_key()
        ss_key = ss%hash_key()
        ! SipHash-2-4 has no internal fixed key.
