! test_easy_chacha20.f90 -- ChaCha20 Easy Mode encryptor coverage.
!
! Mirrors the C binding's test_easy_chacha20.c at the Easy Mode level.
! ChaCha20 ships at a single hash width (256). The test exercises
...
program test_easy_chacha20
  ...
  character(*), parameter :: TEST_NAME = "test_easy_chacha20"
...
contains
  ...
  function pseudo_plaintext(n) result(p)
    ...
    allocate (p(n))
    do i = 1, n
      p(i) = int(iand(int(i - 1, c_int64_t) * 29_c_int64_t + 43_c_int64_t, &
...
end program test_easy_chacha20
