! test_aescmac.f90 -- AES-CMAC-focused low-level cipher coverage.
!
! AES-CMAC is the W128 PRF-grade primitive in the ITB suite, ships at
! a single hash width (128 -- the AES block size), and carries a
! 16-byte AES-128 key. The test program covers the canonical cross-
! binding shape: roundtrip across the 128 / 256 / 512 nonce-bit
...
program test_aescmac
  ...
  character(*), parameter :: TEST_NAME = "test_aescmac"
  character(*), parameter :: HASH_NAME = "aescmac"
  integer,      parameter :: HASH_WIDTH = 128
  integer,      parameter :: HASH_KEY_LEN = 16
...
contains
  ...
  function token_bytes(n) result(out)
    ...
    integer(c_int64_t), save :: ctr = int(z'4EADBEEFCAFEBABE', c_int64_t)
