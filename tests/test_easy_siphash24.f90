! test_easy_siphash24.f90 -- SipHash-2-4 Easy Mode encryptor coverage.
!
...
! accessor surface, plain encrypt / decrypt round-trip, and
! authenticated encrypt / decrypt round-trip with a tampered-
...
program test_easy_siphash24
  ...
  character(*), parameter :: TEST_NAME = "test_easy_siphash24"
...
contains
  ...
  function pseudo_plaintext(n) result(p)
    ...
    allocate (p(n))
    do i = 1, n
      p(i) = int(iand(int(i - 1, c_int64_t) * 31_c_int64_t + 47_c_int64_t, &
...
end program test_easy_siphash24
