! test_blake3.f90 -- BLAKE3-focused low-level cipher coverage.
!
! BLAKE3 ships at a single hash width (256). The test program covers
! the canonical cross-binding shape applied at the libitb low-level
! free-function surface: roundtrip across the 128 / 256 / 512 nonce-
! bit settings, Triple Ouroboros roundtrip across the same axis, the
! authenticated counterpart for both Single and Triple (with a tamper
! step that confirms a flipped bit inside the dynamic header region
! is rejected as STATUS_MAC_FAILURE), a persistence sweep that round-
! trips a saved seed via from_components / hash_key, a plaintext-size
! sweep across small + medium + large payloads, and an invariants
! block (width, hash-key length).
!
...
program test_blake3
  ...
  character(*), parameter :: TEST_NAME = "test_blake3"
...
contains
  ...
  ! Pseudo-random byte generator -- a SplitMix-style LCG that returns
  ! a different sequence on each call within a single program run.
  function token_bytes(n) result(out)
    integer, intent(in) :: n
    integer(itb_byte_kind), allocatable :: out(:)
    integer(c_int64_t), save :: ctr = int(z'1EADBEEFCAFEBABE', c_int64_t)
    integer(c_int64_t) :: state
    integer :: i
    integer(c_int64_t), parameter :: GOLD  = int(z'1E3779B97F4A7C15', c_int64_t)
  ...
  subroutine test_roundtrip_across_nonce_sizes()
    ...
    do i = 1, size(NONCE_SIZES)
      ...
      call new_itb_seed(s0, "blake3", 1024)
      ...
      call assert_true(TEST_NAME, "ct_len >= header_size", &
                        size(ct) >= int(hsize))
  ...
  subroutine test_auth_across_nonce_sizes()
    ...
    do i = 1, size(NONCE_SIZES)
      do m = 1, size(MAC_NAMES)
        ...
        ! Tamper inside the 256-byte window after the fixed-size wire
        ! header. Authenticated decrypt must reject with
        ! STATUS_MAC_FAILURE. Probe via the low-level FFI so the
        ! non-OK status can be observed without `error stop`.
        hsize = itb_header_size()
        end_b = int(hsize) + 256
  ...
  subroutine test_persistence_across_nonce_sizes()
    ...
    character(*), parameter :: HASH_NAME = "blake3"
    ...
    do ki = 1, size(CANDIDATE_KB)
      ...
      do i = 1, size(NONCE_SIZES)
        ...
        call assert_int_eq(TEST_NAME, "noise components count*64 == kb", &
                            size(ns_comps) * 64, kb)

        ns_key = ns%hash_key()
        ds_key = ds%hash_key()
        ss_key = ss%hash_key()
        call assert_int_eq(TEST_NAME, "noise hash_key length", size(ns_key), 32)
  ...
  subroutine test_invariants()
    ...
    width = ns%width()
    call assert_int_eq(TEST_NAME, "blake3 width", int(width), 256)

    got_name = ns%hash_name()
    call assert_string_eq(TEST_NAME, "blake3 hash_name", got_name, "blake3")

    key = ns%hash_key()
    call assert_int_eq(TEST_NAME, "blake3 hash_key length", size(key), 32)

    call ns%destroy(); call ds%destroy(); call ss%destroy()
  end subroutine

end program test_blake3
