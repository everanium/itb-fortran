! test_easy_blake2b.f90 -- BLAKE2b Easy Mode encryptor coverage.
!
! Mirrors the C binding's test_easy_blake2b.c at the Easy Mode level.
! BLAKE2b ships at two hash widths -- blake2b256 (W256) and
! blake2b512 (W512); both are exercised here. Each width is
...
program test_easy_blake2b
  ...
  character(*), parameter :: TEST_NAME = "test_easy_blake2b"
  integer,      parameter :: KEY_BITS  = 1024
  integer,      parameter :: PT_LEN    = 256
  character(len=10), parameter :: HASH_NAMES(2) = &
...
contains
  ...
  function pseudo_plaintext(n) result(p)
    ...
    allocate (p(n))
    do i = 1, n
      p(i) = int(iand(int(i - 1, c_int64_t) * 17_c_int64_t + 31_c_int64_t, &
