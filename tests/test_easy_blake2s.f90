! test_easy_blake2s.f90 -- BLAKE2s Easy Mode encryptor coverage.
!
! Mirrors the C binding's test_easy_blake2s.c at the Easy Mode level.
! BLAKE2s ships at a single hash width (256). The test exercises
...
program test_easy_blake2s
  ...
  character(*), parameter :: TEST_NAME = "test_easy_blake2s"
...
contains
  ...
  function pseudo_plaintext(n) result(p)
    ...
    allocate (p(n))
    do i = 1, n
      p(i) = int(iand(int(i - 1, c_int64_t) * 19_c_int64_t + 37_c_int64_t, &
