...
**link against `libitb3.so` at compile time** (`-litb3` with an
...
The convenience driver builds `libitb3.so` (if absent), the binding's
...
exactly one
`itb_pipeline_free` call (libitb3 zeroes key material on free);
...
- libitb3.so must be reachable at runtime (embedded RPATH points at
