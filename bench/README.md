...
| `ITB_LOCKSEED`       | unset   | When set to a non-empty / non-`0` value, every encryptor in the run calls `e%set_lock_seed(1)`. Easy Mode auto-couples `set_bit_soup(1)` + `set_lock_soup(1)`, so no separate flags are needed. The mixed-primitive cases attach a dedicated lockSeed primitive (via `prim_l`) only under this flag; otherwise `prim_l` is omitted so the no-LockSeed bench arm measures the plain mixed-primitive cost. |
...
single-primitives + 1 mixed x 4 ops) and converges per case in 5-15
