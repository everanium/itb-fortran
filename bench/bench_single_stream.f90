! bench_single_stream.f90 -- Single Ouroboros streaming benchmarks for
! the Fortran binding.
!
...
!
!   make bench
!   ./bench/bin/itb-bench-single-stream
!
!   ITB_BENCH_FILTER=easy_encrypt_aead_io ./bench/bin/itb-bench-single-stream

module bench_single_stream_state
...
contains
  ...
  function alloc_state() result(idx)
    ...
    if (bench_states_len >= MAX_CASES) then
      write (error_unit, "(A)") "bench_single_stream: state registry exhausted"
      error stop 1
    end if
  ...
  subroutine state_destroy_all()
    ...
    do i = 1, bench_states_len
      ...
      call bench_states(i)%data%destroy()
      call bench_states(i)%start%destroy()
...
end module bench_single_stream_state
...
end module bench_single_stream_io


program bench_single_stream
  ...
  use itb_streams,   only: itb_stream_encrypt_auth, itb_stream_decrypt_auth, &
  ...
  use bench_single_stream_state
  use bench_single_stream_io
  implicit none
  ...
  write (output_unit, "(A,I0,A,I0,A,A,A,I0,A,A,A,I0,A,A,A)") &
      "# single_stream payload_bytes=", STREAM_TOTAL_BYTES,             &
...
contains
  ...
  subroutine build_stream_encryptor(idx)
    ...
    call new_itb_encryptor(bench_states(idx)%enc, STREAM_PRIMITIVE,         &
                            STREAM_KEY_BITS, STREAM_MAC_NAME, 1)
  ...
  subroutine build_stream_seeds(idx)
    ...
    call new_itb_seed(bench_states(idx)%data,  STREAM_PRIMITIVE, STREAM_KEY_BITS)
    call new_itb_seed(bench_states(idx)%start, STREAM_PRIMITIVE, STREAM_KEY_BITS)
  end subroutine
  ...
  subroutine run_lowlevel_encrypt_aead_io(case_idx, iters)
    ...
    do i = 1_int64, iters
      ...
      w%len   = 0_c_size_t
      w%data  = bench_states(case_idx)%write_pool_cptr
      call itb_stream_encrypt_auth(bench_states(case_idx)%noise,            &
            bench_states(case_idx)%data, bench_states(case_idx)%start,        &
            bench_states(case_idx)%mac,                                       &
            rfn, c_loc(r), wfn, c_loc(w),                                    &
            int(STREAM_CHUNK_BYTES, itb_size_kind), status)
      if (status /= STATUS_OK) error stop "low-level stream_encrypt_auth failed"
    end do
  end subroutine
  ...
  subroutine run_lowlevel_decrypt_aead_io(case_idx, iters)
    ...
    do i = 1_int64, iters
      ...
      w%len   = 0_c_size_t
      w%data  = bench_states(case_idx)%drain_pool_cptr
      call itb_stream_decrypt_auth(bench_states(case_idx)%noise,            &
            bench_states(case_idx)%data, bench_states(case_idx)%start,        &
            bench_states(case_idx)%mac,                                       &
            rfn, c_loc(r), wfn, c_loc(w),                                    &
            int(STREAM_CHUNK_BYTES, itb_size_kind), status)
      if (status /= STATUS_OK) error stop "low-level stream_decrypt_auth failed"
    end do
  end subroutine
  ...
  subroutine run_lowlevel_encrypt_userloop(case_idx, iters)
    ...
    do i = 1_int64, iters
      ...
      do while (off < int(STREAM_TOTAL_BYTES, itb_size_kind))
        ...
        if (end_off > int(STREAM_TOTAL_BYTES, itb_size_kind))              &
            end_off = int(STREAM_TOTAL_BYTES, itb_size_kind)
        ct = itb_encrypt(bench_states(case_idx)%noise,                       &
                          bench_states(case_idx)%data,                        &
                          bench_states(case_idx)%start,                        &
                          bench_states(case_idx)%payload(int(off) + 1:int(end_off)))
        call frame_chunk(transcript_buf, transcript_len, ct)
        deallocate (ct)
        off = end_off
  ...
  subroutine run_lowlevel_decrypt_userloop(case_idx, iters)
    ...
    do i = 1_int64, iters
      ...
      do while (off + 4_itb_size_kind <= total_len)
        ...
        pt = itb_decrypt(bench_states(case_idx)%noise,                       &
                          bench_states(case_idx)%data,                        &
                          bench_states(case_idx)%start,                        &
                          bench_states(case_idx)%transcript(int(off) + 1:int(off + ct_len)))
        do j = 1_itb_size_kind, int(size(pt), itb_size_kind)
  ...
  subroutine prebuild_lowlevel_aead_transcript(idx)
    ...
    w%len   = 0_c_size_t
    w%data  = c_loc(pool)
    call itb_stream_encrypt_auth(bench_states(idx)%noise,                   &
          bench_states(idx)%data, bench_states(idx)%start,                    &
  ...
  subroutine prebuild_lowlevel_userloop_transcript(idx)
    ...
    do while (off < int(STREAM_TOTAL_BYTES, itb_size_kind))
      ...
      if (end_off > int(STREAM_TOTAL_BYTES, itb_size_kind))                &
          end_off = int(STREAM_TOTAL_BYTES, itb_size_kind)
      ct = itb_encrypt(bench_states(idx)%noise,                              &
                        bench_states(idx)%data,                               &
                        bench_states(idx)%start,                               &
                        bench_states(idx)%payload(int(off) + 1:int(end_off)))
      call frame_chunk(transcript_buf, transcript_len, ct)
      deallocate (ct)
      off = end_off
  ...
  subroutine build_cases(cs, n_out)
    ...
    character(*), parameter :: PREFIX = "bench_single_stream_areion512_1024bit_64mb"
    idx = 0
    idx = idx + 1
...
end program bench_single_stream
