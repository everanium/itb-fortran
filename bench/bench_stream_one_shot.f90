...
program bench_stream_one_shot
  ...
  ! case: itb_encrypt_stream_one_shot_into grows it once to the
  ...
  if (.not. itb_ok(err)) then
    write (error_unit, '(a)') "bench_stream_one_shot: init failed: "// &
        itb_error_text(err)
    error stop 1
  end if
  ...
  do i = 1, size(sizes)
    ...
    call itb_encrypt_stream_one_shot_into(pipe, plain, dec_wire, n_dec_wire, err)
    if (.not. itb_ok(err)) then
      write (error_unit, '(a)') "bench_stream_one_shot: dec setup encrypt failed: "// &
          itb_error_text(err)
      error stop 1
    end if
...
contains
  ...
  subroutine run_case(size_bytes)
    ...
    call run_stream_one_shot(ok)
    if (.not. ok) then
      write (error_unit, '(a)') "bench_stream_one_shot: warm-up failed"
      error stop 1
    end if
    start = bench_now()
    elapsed = 0.0_real64
    ...
    do while (elapsed < budget .or. iters < BENCH_MIN_ITERS)
      call run_stream_one_shot(ok)
      if (.not. ok) then
        write (error_unit, '(a)') "bench_stream_one_shot: iteration failed"
        error stop 1
      end if
      iters = iters + 1
      elapsed = bench_now() - start
    end do
    call bench_report("stream_one_shot", size_bytes, iters, elapsed)
  end subroutine

  subroutine run_stream_one_shot(ok)
    logical, intent(out) :: ok
    type(itb_error_t) :: run_err

    call itb_encrypt_stream_one_shot_into(pipe, plain, wire, n_wire, run_err)
    ok = itb_ok(run_err)
  end subroutine
  ...
  subroutine run_dec_case(size_bytes)
    ...
    call run_stream_one_shot_dec(ok)
    if (.not. ok) then
      write (error_unit, '(a)') "bench_stream_one_shot: dec warm-up failed"
      error stop 1
    end if
    start = bench_now()
    elapsed = 0.0_real64
    ...
    do while (elapsed < budget .or. iters < BENCH_MIN_ITERS)
      call run_stream_one_shot_dec(ok)
      if (.not. ok) then
        write (error_unit, '(a)') "bench_stream_one_shot: dec iteration failed"
        error stop 1
      end if
      iters = iters + 1
      elapsed = bench_now() - start
    end do
    call bench_report("stream_one_shot-dec", size_bytes, iters, elapsed)
  end subroutine

  subroutine run_stream_one_shot_dec(ok)
    logical, intent(out) :: ok
    type(itb_error_t) :: run_err

    call itb_decrypt_stream_one_shot_into(pipe, dec_wire(1:n_dec_wire), dec_out, &
        n_dec_out, run_err)
    ok = itb_ok(run_err)
  end subroutine
